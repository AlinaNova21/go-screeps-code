...
(() => {
  ...
  const dummyProxy = mod => new Proxy({}, {
    get(target, name) {
      console.log(`Dummy mod ${mod}`)
      // console.log(`Dummy ${name} called on ${mod}`)
      return enosys
    }
  })
  ...
  if (!global.crypto) {
    ...
    const nodeCrypto = dummyProxy("crypto")
    global.crypto = {
      getRandomValues(b) {
  ...
  global.Go = class {
      ...
      this.importObject = {
        wasi_unstable: {
          ...
          fd_write: function (fd, iovs_ptr, iovs_len, nwritten_ptr) {
            ...
            if (fd == 1) {
              ...
              console.log('invalid file descriptor:', fd);
            }
        ...
        env: {
          ...
          "syscall/js.valueCall": (ret_addr, v_addr, m_ptr, m_len, args_ptr, args_len, args_cap) => {
            ...
            // console.log(`valueCall ${name}(${args.join(', ')})`)
            try {
    ...
    run(instance) {
      ...
      this._resolveCallbackPromise = () => console.log('resolveCallbackPromise')
